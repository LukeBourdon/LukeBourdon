...
The 16 scenarios comprise **4 Critical, 10 High, 2 Medium and 0 Low current risks** under the locally defined matrix. These are scenario judgments, not empirically estimated probabilities. All treatments remain proposed.
...
Sensitivity: R03 has current L4/I4 = 16; if observation confirms fewer exports and effective restrictions, L3 yields 12, still High. R12 has L2/I4 = 8; if encryption evidence fails, increasing L to 3 produces 12, requiring escalation. Obtain those facts before accepting risk. Do not interpret the target scores as a promised reduction or add overlapping scores together.

Success is verified reduction in current exposure supported by tests, rather than the number of documents produced. Report expired exceptions and failed retests alongside completed actions.
